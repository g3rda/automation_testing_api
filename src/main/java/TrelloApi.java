...
public interface TrelloApi {
    ...
    @POST("/1/lists")
    Call<List> createList(@Query("name") String name, @Query("idBoard") String id, @Query("key") String key, @Query("token") String token);

    @POST("/1/cards")
    Call<Card> createCard(@Body Card card, @Query("key") String key, @Query("token") String token);

    @PUT("/1/cards/{id}")
    Call<Card> updateCard(@Body Card card, @Path("id") String id, @Query("key") String key, @Query("token") String token);
